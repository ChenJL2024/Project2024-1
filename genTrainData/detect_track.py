...
from utils.general import check_img_size, check_requirements, check_imshow, non_max_suppression_delt, apply_classifier, \
...
def detect(opt):
    ...
    for frame_idx, (path, img, im0s, vid_cap) in enumerate(dataset):
        ...
        print(pred[...,4].max())
        # Apply NMS
        pred = non_max_suppression_delt(pred, opt.conf_thres, opt.iou_thres, classes=opt.classes, agnostic=opt.agnostic_nms, kpt_label=kpt_label,nc = model.yaml['nc'],nkpt=model.yaml['nkpt'])
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for k in range(0,det.shape[0]):
                    xyconf_key = tuple(det[k,:5])
                    keypoint_value0 = det[k,5:]
                    before_dict_origin[xyconf_key] = keypoint_value0
                    # print(det[k,:4].numpy().astype(int))
                    int_key = tuple(det[k,:4].numpy().astype(int))
                    keypoint_value1 = det[k,4:]
                    int_dict[int_key] = keypoint_value1
                ...
                for det_index, (output, conf) in enumerate(zip(outputs, det[:, 4])):
                    ...
                    xyxy = output[0:4]
                    print(type(output))
                    tracked_key = tuple(xyxy)
                    tracked_value = int_dict[tracked_key]
                    id = output[4]
                    newid_xy = np.concatenate(([id], xyxy))
                    newtracked_key = tuple(newid_xy)
                    tracked_dict[newtracked_key] = tracked_value
                    ...
                    if save_img or opt.save_crop or view_img:
                        ...
                        # print(kpts)
                        # print(tracked_dict[tracked_key][2:])
                        kpts = tracked_dict[newtracked_key][2:]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default='peep_20s.mp4', help='source')  # file/folder, 0 for webcam

...
# listFiles = ['传递右','传递左','03L-传递纸条1115','03R-传递纸条1115','03L-传递纸条','03R-传递纸条']
...
ceilNumSamplesPerStu = 12   #对一个学生样本所作的总采样个数上限，最小1
...
def retrieveLabel(file, numSamples, fps):
    fileGen = read_srt_file_gen('../srt/passon_clear/' + file + '.srt')
    lenTime = 0.0
    sampleDict = dict()
...
if __name__ == '__main__':
    ...
    for key in tqdm.tqdm(dataDict.keys()):
        ...
        # # Save
        np.save('../out/' + key + '.npy', transformed_keypoints_array.transpose(1, 3, 0, 2))  # shape: N,C,T,V
        # np.save('../out/' + key + '.npy', transformed_keypoints_array)  # shape: N,C,T,V
    print('\nAll Done.')

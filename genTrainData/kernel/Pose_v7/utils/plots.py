...
from kernel.Pose.utils.general import xywh2xyxy, xyxy2xywh
from kernel.Pose.utils.metrics import fitness
...
def plot_one_box(x, cla, im, color=None, label=None, line_thickness=3, kpt_label=False, kpts=None, steps=2, orig_shape=None):
    # Plots one bounding box on image 'im' using OpenCV
    assert im.data.contiguous, 'Image not contiguous. Apply np.ascontiguousarray(im) to plot_on_box() input image.'
    tl = line_thickness or round(0.002 * (im.shape[0] + im.shape[1]) / 2) + 1  # line/font thickness
    color = 140 #color or [random.randint(0, 255) for _ in range(3)]
    ...
    #         cv2.rectangle(im, c1, c2, color, -1, cv2.LINE_AA)  # filled c1左上角坐标，c2右下角坐标
    ...
    if label:
        ...
        if cla == 0:
            ...
            label = label+'t'
            cv2.putText(im, label, (c2[0], c2[1] - 2), 0, tl / 6, [225, 0, 0], thickness=tf//2, lineType=cv2.LINE_AA)
    if kpt_label:
        plot_skeleton_kpts(im, kpts, steps, c1, c2,orig_shape=orig_shape)


def plot_skeleton_kpts(im, kpts, steps, c1, c2, orig_shape=None):
    # print("ccccccccccccccc: ",c1,c2)
    ...
    pose_limb_color = palette[[9, 10, 11, 9, 13, 14, 15, 16, 17, 18, 19, 8, 6, 3, 4, 2, 1, 5, 7]]
    ...
    for kid in range(num_kpts):
        ...
        # if not (x_coord % 640 == 0 or y_coord % 640 == 0): # 如果关键点是0的话这里就不会往下走
        #     if steps == 3:
        #         conf = kpts[steps * kid + 2]
        #         if conf < 0.3:
        #             continue
            # if int(y_coord)>int(c1[1]):
        cv2.circle(im, (int(x_coord), int(y_coord)), radius, (int(r), int(g), int(b)), -1)
    order_list = [[6, 9], [9, 21], [21, 12], [12, 15], [15, 18], [21, 24], [21, 27], [27, 30], [30, 33]]
    order_list = [[0, 3], [3, 15], [15, 6], [6, 9], [9, 12], [15, 18], [15, 21], [21, 24], [24, 27]]
    # order_list = [[1, 4], [4, 16], [16, 7], [7, 10], [10, 13], [16, 19], [16, 22], [22, 25], [25, 28]]
    # skeleton = order_list

    # skeleton = [ [1, 2],[2,6], [6,4], [4, 5], [6, 7], [6, 8], [8, 9], [9, 10],
    #              [1,3], [1, 8], [1, 4], [1, 9], [1, 5],
    #             [1, 10], [2, 3], [2, 8], [6, 3], [6, 8], [3, 5]]

    skeleton = [[1, 2], [2, 6], [3, 4], [6, 4], [4, 5], [6, 7], [6, 8], [8, 9], [9, 10],
                [1, 3], [1, 8]]
    skeleton = [[1, 2], [2, 6], [6, 3], [6, 8], [3, 4], [4, 5], [6, 7], [8, 9], [9, 10]]

    # for i in order_list:
    #     print(i)
    #     show_kpts = kpts
    #     pos1 = (int(kpts[i[0]-6]), int(kpts[i[1]-6] + 1))
    #     pos2 = (int(kpts[i[1]-6]), int(kpts[i[1]-6] + 1))
    #     cv2.line(im, pos1, pos2, (0, 0, 255), thickness=2)
    #     # cv2.imshow("fdsf", im)
    #     # cv2.waitKey(0)

    for sk_id, sk in enumerate(skeleton): #讲各个关键点连接起来
        r, g, b = pose_limb_color[sk_id]
        pos1 = (int(kpts[(sk[0]-1)*steps]), int(kpts[(sk[0]-1)*steps+1]))
        pos2 = (int(kpts[(sk[1]-1)*steps]), int(kpts[(sk[1]-1)*steps+1]))
        # print("\nsk_id %s,sk %s,sk[0] %s,steps %s,pos1 %s,pos2 %s"%(sk_id,sk,sk[0],steps,pos1,pos2))
        if steps == 3:
        ...
        #     continue
        # if pos1[1] > int(c1[1]) and pos2[1] > int(c1[1]) :
            # cv2.circle(im, (int(x_coord), int(y_coord)), radius, (int(r), int(g), int(b)), -1)

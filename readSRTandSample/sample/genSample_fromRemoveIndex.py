    ...
    while True:
      try:
        ...
        if "--> " in item:
          ...
          dataValid=series[start:end+1:interval_inSample]
...
if __name__=='__main__':
  ...
  while True:
      ...
      if line:
        #print(line)
        rest = patrnTitle.search(line)
        if rest:
            indexFile=rest.group(1)
            line = f.readline()
            l=line.split(',')
            temp=[]
            #print(indexFile)
            ...
            for s in l:
                #print(s)
                temp.append(int(s))
            removeDict[indexFile]=temp
      else:
          break
  f.close()
  print(f'len of removeDict: {len(removeDict)}')
  #print(removeDict)
  ...
  for file in tqdm.tqdm(listFiles):
      ...
      if file in removeDict:
          index=list(set([i for i in range(series.shape[1])]) - set(removeDict[file]))
      else:
          index=list(set([i for i in range(series.shape[1])]))
      index.sort()
      lableDict,lenTimeTemp=retrieveLable(lableDict,file,series[:,index,:,:],frameInterval,fps)
      if lenTimeTemp>lenTime:
         lenTime=lenTimeTemp

...
from kernel.Pose.utils.google_utils import gsutil_getsize
from kernel.Pose.utils.metrics import fitness
from kernel.Pose.utils.torch_utils import init_torch_seeds
...
def non_max_suppression_v8(
    ...
    prediction_copy[..., :4] = xywh2xyxy(prediction_copy[..., :4])  # xywh to xyxy

    t = time.time()
    output = [torch.zeros((0, 6 + nm), device=prediction.device)] * bs
    for xi, x in enumerate(prediction_copy):  # image index, image inference
        # Apply constraints
        ...
        if (time.time() - t) > time_limit:
            print(f'WARNING ⚠️ NMS time limit {time_limit:.3f}s exceeded, test reason ignore 20240508 novelian')
            # LOGGER.warning(f'WARNING ⚠️ NMS time limit {time_limit:.3f}s exceeded')
            # break  # time limit exceeded

    return output

def non_max_suppression_delt(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    xc = prediction[..., 4] > conf_thres  # candidates ,取每一个最后一维的第五个数据也就是置信度
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            if not kpt_label:
                ...
                # 删除老师的信息
                x = np.delete(x.cpu(),np.where(x.cpu()[:,5] == 1),axis=0)

...
peep_listFiles = ['01L-旁窥1115_series','01L-旁窥_series','01R-旁窥1115_series','01R-旁窥_series','suda_left_peep_1_series','suda_right_peep_1_series','182_left_peep_1_series','188_right_peep_1_series'] # 12
...
back_listFiles = ['02L-回头1_series','02L-回头2_series','02L-回头1115_series','02R-回头1_series','02R-回头2_series','02R-回头1115_series', 'suda_left_back_1_series', 'suda_right_back_1_series', '182_left_back_1_series', '188_right_back_1_series'] # 14
...
passon_listFiles = ['182_left_passon_1_series','182_left_passon_2_series','182_left_passon_3_series',
                    ...
                    'suda_right_passon_1_series','suda_right_passon_2_series'] # 16
...
raise_listFiles = ['05L_raise1_series','05L_raise2_series','05R_raise1_series','05R_raise2_series','suda_left_raise_1_series','suda_right_raise_1_series'] # 20
...
stand_listFiles = ['suda_left_stand_1_series','suda_left_stand_2_series','suda_right_stand_1_series','suda_right_stand_2_series', 
                   '182_left_stand_1_series','182_left_stand_2_series','188_right_stand_1_series','188_right_stand_2_series'] # 12
...
class oneSample: # 处理一个目标从头到尾的所有样本
    ...
    def gen_onePieceData(self, dataTemp, start, end, key, file_name, beha_label):
        ...
        save_path = f'./out_raise/{beha_label}/{file_name}/{key}' # stand、raise
        # save_path = f'./out_raise/{file_name}/{key}' # peep、back、passon、
        npy_file_path = process_save_datavalid(save_path, dataValid, self.object_order)
        # 如果save_path 中没有该文件，不做下面的操作
        # positive_npy_file_path = f'{save_path}/positive/{self.object_order}.npy' # peep、back、passon
        # negative_npy_file_path = f'{save_path}/negative/{self.object_order}.npy' # peep、back、passon
        # if not os.path.exists(positive_npy_file_path) and not os.path.exists(negative_npy_file_path): # peep、back、passon
        
        positive_npy_file_path = f'{save_path}/{self.object_order}.npy' # stand、raise
        if not os.path.exists(positive_npy_file_path): # stand、raise
            print('被过滤掉的文件：', positive_npy_file_path)
            self.object_order += 1
            return None
        print('要用到的文件：', positive_npy_file_path)
    ...
    def doRandomClipAndAmpli(self, data, start, end):
        ...
        dataNew = dataNew[:, ::6,:,:] # 每隔2个抽取一个## 15s的时间片段，如果逐帧滑动产生的数据会非常多，将最终拿到的数据除以3
        print('#2:',dataNew.shape)
        return dataNew 
...
if __name__ == '__main__':
    ...
    # npy_root_path = '/home/goldsun/data/genData/npy/peep'
    ...
    for key in tqdm.tqdm(dataDict.keys()):
        ...
        data_copy = data.copy()
        np.save('./out/' + key + '_original.npy', data_copy)  # shape: N,C,T,V
        # if key == 'normal':
            # continue
        # 归一化

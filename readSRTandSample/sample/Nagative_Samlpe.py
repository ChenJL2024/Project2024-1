...
listFiles = ['01L-旁窥', '01L-旁窥1115', '01R-旁窥', '01R-旁窥1115']#'旁窥左','旁窥右']
...
ceilNumSamplesPerStu = 36   #对一个学生样本所作的总采样个数上限，最小1
...
def retrieveLabel(file, numSamples, fps):
    fileGen = read_srt_file_gen('../srt/peep_clear/' + file + '.srt')
    lenTime = 0.0
    sampleDict = dict()
    ...
    while True:
        try:
            ...
            if "--> " in item:
                ...
                box = next(fileGen).replace(" ", "").replace("\n", "")
                nextLine = next(fileGen).replace(" ", "").replace("\n", "")
                if nextLine=="":# or nextLine != "":
                    for key in sampleDict.keys():
                        sampleDict[key].append((box, start, end))
                    continue
                    ...
                    if nextLine[:1]=="a":
                       ...
                       for index in addList:
                           ...
                           if obj in sampleDict:
                               sampleDict[obj].append((box,start,end))
                    elif nextLine[:1] == 'd':
                        validFlag = True
...
if __name__ == '__main__':
    ...
    for file in listFiles:
        ...
        sampleDict,lenTimeTemp = retrieveLabel(file, series.shape[1], FPS_extractedVideo)
        # print(sampleDict)
        startBias=0
        if lenTimeTemp > lenTime:
            lenTime = lenTimeTemp

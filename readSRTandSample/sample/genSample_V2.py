...
ceilNumSamplesPerStu = 4   #对一个学生样本所作的总采样个数上限，最小1
...
class oneSample:
    ...
    def doRandomClipAndAmpli(self, data, start, end):
        ...
        validLen = clipLen
        # print("---1-----", totalLen, validLen)
        dataNew = np.zeros((totalTimeLength, 1, 10, 2))
        total_len = min(totalTimeLength, totalLen)
        start_valindInNew = random.randint(0, min((total_len - validLen), start))
        # print("---2-----", total_len, start_valindInNew)
        ...
        totalLen = total_len - availStart
        totalLen_InNew = totalLen
        # print("---3-----", availStart, totalLen)
        ...
        if totalLen < totalTimeLength:
            ...
            availstart_InNew = 0
            if totalTimeLength > totalLen_InNew:
                availstart_InNew = random.randint(0, totalTimeLength - totalLen_InNew)
        print("--------", availStart, availStart + totalLen)

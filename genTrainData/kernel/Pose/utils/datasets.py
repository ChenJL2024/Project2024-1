...
from kernel.Pose.utils.general import check_requirements, xyxy2xywh, xywh2xyxy, xywhn2xyxy, xyn2xy, segment2box, segments2boxes, \
    resample_segments, clean_str
from kernel.Pose.utils.torch_utils import torch_distributed_zero_first
...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, stride=32):
        ...
        if fps != 25:
            print('视频帧率：========：',fps)
            fps = 25
        self.framInterval = fps /3
        assert self.framInterval>0.0,'Wrong framInterval.'
        if self.framInterval<1.0:
            self.framInterval=1
        else:
            self.framInterval=int(self.framInterval)
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            #self.cap.set(cv2.CAP_PROP_POS_FRAMES, self.frame * self.framInterval + 1)
            if self.frame>0:
                frameForExtract=1
                while frameForExtract % self.framInterval != 0:  #抽帧管理，每8帧抽一帧
                    self.cap.grab()
                    frameForExtract += 1
            self.cap.grab()
            ret_val, img0 = self.cap.retrieve()  ## 到最后读取视频结束后，ret_val返回false说明已经结束

            t1 = datetime.datetime.now()
            # print('timeConsuming of next: {:.2f}ms'.format(0.001 * (t1 - t0).microseconds))
            #t1 = datetime.datetime.now()
            #print(f'Time Elapsed {0.001 * (t1 - t0).microseconds}ms')
            ...
            if not ret_val:
                ...
                if self.count == self.nf:  # last video
                    #raise StopIteration
                    return -1, np.zeros((3,384, self.img_size),dtype=np.uint8), np.zeros((1080,1920,3),dtype=np.uint8), 0, self.frame
                else:
                    path = self.files[self.count]
                    self.new_video(path)
        ...
        # Padded resize
        img = LetterBox(self.img_size, auto=True, stride=32)(image=img0) # 640x640，384x640
        # Convert
        img = img[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
        img = np.ascontiguousarray(img)
        print('================================================:',self.frame)
        return path, img, img0, self.cap, self.frame  # img: 3x640x640, uint8
